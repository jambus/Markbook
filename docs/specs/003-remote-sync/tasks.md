...
- [x] T318 Android：以提供方无关的持久任务状态与前台后台服务承载 Google Drive 同步；
  离开页面后仍可继续编辑，完成/失败通知与应用内详情共用状态模型（FR-319）。
- [x] T319 Android：远端仅下载在最终提交前发现本地同路径文件时保留远端冲突副本，并将
  同步范围收敛为 Markdown 与 `assets/`，避免同步期间覆盖本地新内容（FR-320）。
