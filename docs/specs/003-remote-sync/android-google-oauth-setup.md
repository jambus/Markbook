...
1. In Google Cloud Console, create a project named `禾记（Heji Notes）` and enable the
   Google Drive API.
2. In Google Auth Platform, complete Branding and choose an External audience.
   ...
   test user.
3. Under Data Access, declare `https://www.googleapis.com/auth/drive`. 禾记（Heji Notes）
...
The supplied Android client must be registered for `com.jambus.heji`. The app
